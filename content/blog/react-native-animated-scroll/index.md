---
title: A definitive guide to collapsible header effect using the React Native Animated API
...
This is a pretty standard and commonly used effect in mobile apps. But we will take a slightly different approach in this post to achieve the desired effect. **We take a look at the React Native source code!**
...
Since most of them are generic graphics techniques and if you're not familiar with them beforehand, writing animation would be a more daunting experience.
...
Our `CollapsibleHeader` component will need to know about the scroll value in order to function, therefore we will add this prop to the component which is in the container component:
...
What is next, is one of the crucial steps of achieving our desired animated effect: The `diffClamp`.

To understand what does this `Animated` function do, let's start with clamping itself.
...
You can think of interpolation as a mapping function. We declare what are the starting and ending values (which is the `inputRange`), and then what we want to convert these values into (which is the `outputRange`).
...
Now let's talk about the chosen range. We start the input range from `0`, since we want the calculations to start at the top of the list when no motion is done yet. And we stop the range when the user scrolls about the height of the header. Since we want to display the sticky bar all the way around, we're subtracting the height of the bar here.
To get the sticky bar height, we've got several solutions. The solution used here exposes the `setStickyHeight` method to the parent, and the parent passes it to the sticky bar component and then this function gets called in the Navbarcomponent's `onLayout` function eventually and gives us the height.
...
The output range in our case is just reversing the value. Since we want to hide the header when the user scrolls down and thus the `scrollY` value increases. And we want to display the header as soon as the user scrolls up. (therefore decreasing the `scrollY` value).
...
And we're good. Now we should have a smooth appearing/disappearing animation effect on our header component using the `Animated` API.
